...
kubectl rollout status deployment/darwin-core -n "$NAMESPACE" --timeout="${HEALTH_CHECK_TIMEOUT}s" || {
    echo "❌ Rollout failed!"
    echo ""
    echo "📊 Pod status:"
...
POD_NAME=$(kubectl get pods -n "$NAMESPACE" -l app=darwin-core -o jsonpath='{.items[0].metadata.name}')
...
echo ""
echo "📊 Status:"
kubectl get pods -n "$NAMESPACE" -l app=darwin-core
echo ""
echo "🔗 Service:"
kubectl get svc -n "$NAMESPACE" -l app=darwin-core
echo ""
echo "📋 To view logs:"
echo "  kubectl logs -f deployment/darwin-core -n $NAMESPACE"
echo ""
echo "🌐 To access locally:"
echo "  kubectl port-forward svc/darwin-core 8000:8000 -n $NAMESPACE"
echo ""
echo "══════════════════════════════════════════════════════════════════════"
echo ""
